...
/***************************CREATE*********************************/
$('#registrarPerfil').submit(function(e){
    e.preventDefault();

    var datos = $('#registrarPerfil');
   ...
   $.post(url,datos.serialize(),function (result) {
    ...
    }).success(function(data)
    {
        if($.isEmptyObject(data.error)){
            console.log(data)
            $('#modal-perfil-create').modal('hide')
            $('#modal-exito').modal({
                backdrop: 'static',
                keyboard:false
            });
...
//		alert("hola");
      $('#modal-perfil-create').modal('show');
});
...
$('body').on('click','.edit',function(e){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $('#modal-perfil-edit input[name=editnombre]').val(data.descripcion)

            $('#modal-perfil-edit').modal('show');
        },
        error: function(data){
...
$('#update').on('click',function (e) {
    ...
    var url = $('#editarPerfil').attr('action');
